  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      ...
      if (filter_var($item->url, FILTER_VALIDATE_URL)) {
        ...
        array_push($tags, 'iframe', 'div', 'h2', 'h3', 'h4', 'h5', 'h5', 'h6', 'footer', 'article', 'table', 'tbody', 'th', 'td', 'tr', 'aside', 'figure', 'figcaption', 'section');
        while (preg_match('/<iframe[^>]*\/>/', $output, $matches, PREG_OFFSET_CAPTURE)) {
          $output = substr_replace($output, "> </iframe>", strlen($matches[0][0])+$matches[0][1]-2, 2);
        }
